  ...
  useEffect(() => {
    const handleFacebookCallback = async () => {
      try {
        ...
        // Call the Facebook callback API
        const response = await fetch(`/api/auth/facebook/callback?code=${code}&state=${state || ''}`);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <LoadingSpinner />
        <p className="mt-4 text-gray-600">Completing sign in with Facebook...</p>
      </div>
    </div>
  );
}